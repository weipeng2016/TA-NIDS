...
def dataframe_to_matrix(df, labelindex=0, startcol=1):
    ...
    truelabels = np.array([df.iloc[i, labelindex] for i in range(df.shape[0])], dtype=int)
...
def generate_csv_writer(csv_path):
    ...
    csv_writer.writerow([str(10*i) for i in range(1, 101)])
    csv_file.flush()
